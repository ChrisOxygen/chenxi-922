...
export default function PricingCTA({ variant = "default", arrowColor = "white", location }: Props) {
  ...
  function handleClick() {
    ph?.capture(EVENTS.CTA_CLICKED, { location: location ?? variant });
    startTransition(() => {
  ...
  return (
    <div className="relative w-full sm:max-w-2xl">
      ...
      <button
        onClick={handleClick}
        disabled={isPending}
        className="gold-cta flex flex-col items-center gap-2 text-brand-primary-dark font-heading px-6 sm:px-10 md:px-14 py-4 w-full disabled:opacity-80 disabled:cursor-not-allowed"
      >
        {isPending ? (
          <span className="flex items-center gap-2.5 py-2">
            <Spinner className="size-5 border-[2.5px] text-brand-primary-dark" />
          ...
          <>
            <span className="text-xs line-through sm:text-sm md:text-base font-medium text-center">
            ...
            <span className="text-xs sm:text-sm md:text-base text-brand-primary font-medium text-center">
